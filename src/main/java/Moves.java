...
public class Moves {
    ...
    // Key: Pair of (delta row, delta column) indicating relative move direction and distance
     ...
     * @param occupiedCells Set of occupied cells (as Pair objects)
    ...
    private boolean pathIsClear(int[] srcCell, int[] dstCell, java.util.Set<Pair> occupiedCells) {
        ...
        return true;  // No blocking pieces found on the path
    }

    /**
     * Inner static class representing a pair of integers (row, col).
     * Used as a key for move offsets and board cell positions.
     */
